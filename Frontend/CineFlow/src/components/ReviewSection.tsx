...
const ReviewSection: React.FC<Props> = ({ contentId, contentType, currentUserId }) => {
  ...
  const fetchReviews = async () => {
    const res = await axios.get(`${BACKEND_URL}/reviews/${contentType}/${contentId}`);
  ...
  const handleAddReview = async () => {
    ...
    await axios.post(`${BACKEND_URL}/reviews/add`, {
  ...
  const handleDeleteReview = async (id: number) => {
    await axios.delete(`${BACKEND_URL}/reviews/delete/${id}`);
    fetchReviews();
  };
  ...
  const handleUpdateReview = async (review: Review) => {
    ...
    await axios.put(`${BACKEND_URL}/reviews/update/${review.id}`, {
        ...
        reviews.map((r) => (
          <div key={r.id} className="border border-gray-700 p-3 rounded mb-3 bg-gray-900">
            <div className="flex items-center gap-3">
              <img
                src={`${BACKEND_URL}${r.profile_image}`}
                alt={r.username}
