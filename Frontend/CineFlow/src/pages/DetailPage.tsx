...
const DetailPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND_URL}/watchlist/${userId}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      try {
        ...
        const detailRes = await fetch(`${BACKEND_URL}/api/${type}/${id}`);
        const detailData = await detailRes.json();
        setContent(detailData);

        const creditsRes = await fetch(
          `https://api.themoviedb.org/3/${isMovie ? 'movie' : 'tv'}/${id}/credits?api_key=${TMDB_API_KEY}&language=tr-TR`
        );
        const creditsData = await creditsRes.json();
        setCast(creditsData.cast?.slice(0, 8) || []);

        const trailerRes = await fetch(`${BACKEND_URL}/trailer/${isMovie ? 'movie' : 'tv'}/${id}`);
  ...
  useEffect(() => {
    fetch(`${BACKEND_URL}/api/category/movie?sort_by=imdb&page=${getRandomPage(10)}`)
      .then(res => res.json())
      .then(data => setTopMovies(data.results?.slice(0, 20) || []));
    fetch(`${BACKEND_URL}/api/category/tv?sort_by=imdb&page=${getRandomPage(10)}`)
      .then(res => res.json())
      .then(data => setTopSeries(data.results?.slice(0, 20) || []));
    fetch(`${BACKEND_URL}/api/category/movie?sort_by=year&page=${getRandomPage(10)}`)
      .then(res => res.json())
      .then(data => setNewMovies(data.results?.slice(0, 20) || []));
    fetch(`${BACKEND_URL}/api/category/tv?sort_by=year&page=${getRandomPage(10)}`)
      .then(res => res.json())
